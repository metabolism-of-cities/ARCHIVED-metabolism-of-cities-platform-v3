...
def referenceform_multiplicity(request, id):
    ...
    if "gps" in request.GET:
        from multiplicity.models import ReferenceSpaceLocation
        gps = Tag.objects.filter(gps__isnull=False, parent=4)
        for details in gps:
            find = ReferenceSpace.objects.filter(name=details.name)
            if find and details.gps != "":
                coords = details.gps.split(',')
                city = find[0]
                if city.location and not city.location.lat:
                    city.location.lat = coords[0].strip()
                    city.location.lng = coords[1].strip()
                    print("Updated for "+city.name)
                elif not city.location:
